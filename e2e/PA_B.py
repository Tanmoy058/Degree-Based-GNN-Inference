...
def inference(feats_file_name, input_queue, graph, pointers, infer_en, arrival_times_actual, num_requests, barrier, arrival_order, res, in_degrees, feats, num_layers, opt, sort, chunk_size, cache_size, GNN, verbose, nsz, lat):
    ...
    n_h = 0
    n_t = 0
    input_features = torch.empty((max(int(chunk_size/5) + 4500000, 5000000), feats.shape[1]), dtype = feats.dtype, device = 'cuda:0')
    torch.cuda.synchronize()
...
if __name__ == '__main__':
    ...
    arrival_rates = [100, 150, 175, 200, 219]
    arrival_rates = [100, 150, 200, 210, 219]
    arrival_rates = []
    ...
    with open(file_name, 'rb') as f:
        cnds = pickle.load(f)
    GNN = 'GAT'
    arrival_rates = [10, 20, 25, 30, 34, 36, 37]
    arrival_rates = [10, 20, 30, 34, 36, 37]
    res = torch.zeros((len(arrival_rates), 10), dtype = torch.float)
    temp = torch.zeros(cnds.shape[1], 9)
    for i in range(len(arrival_rates)):
        print('\n')
