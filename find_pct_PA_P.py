...
def client(max_id, number_of_requests, arrival_model, arrival_rate, HOST, PORT, WIDTH, LAST, verbose):
    ...
    input_queue = np.zeros(number_of_requests, dtype = np.int64)
    input_queue[:] = np.random.choice(max_id, size = number_of_requests, replace = True)
    request_intervals = np.zeros((CVs.shape[0], number_of_requests))
    for i in range(CVs.shape[0]):
        request_intervals[i,:] = np.random.default_rng().gamma(shape = alphas[i], scale = thetas[i], size = number_of_requests)
...
def inference(feats_file_name, input_queue, graph, pointers, infer_en, arrival_times_actual, num_requests, barrier, arrival_order, res, in_degrees, feats, num_layers, opt, sort, chunking, cache_size, GNN, verbose, nsz, chunk_size):
    if verbose:
    ...
    failure = False
    input_features = torch.empty((nsz, feats.shape[1]), dtype = feats.dtype, device = 'cuda:0')
    try:
        with torch.no_grad():
            while infer_en[0]:
                while infer_en[0]:
                ...
                #(plc1, plc1_g, plc2, plc2_c) = cache_inds(ins, cache_meta)
                try:
                    hr[0] = hr[0] + ins.numel()
                    hr[1] = hr[1] + plc1.numel()
                except:
                    hr[0] = hr[0] + 1
                    hr[1] = hr[1] + 1  
                (ins, plc1, plc1_g, plc2, plc2_c) = minibatch[3]
                if ins.shape[0] < input_features.shape[0]:
                    if ins.shape[0] > 6000000:
                        gc.collect()
                        torch.cuda.empty_cache()          
                ...
                #blocks = [b.to(device = 'cuda:0') for b in minibatch[2]]
                    blocks = minibatch[2]
                    torch.cuda.synchronize()
                    t2 = time.perf_counter()
                    preds = model(blocks, input_features[0:ins.shape[0]])
    ...
    if not(failure):
        ...
        print('Server: average latency = ', int(np.mean(torch.bitwise_right_shift(response_times - arrival_times_actual, 20).detach().numpy())), '(ms) failures =', e, [Ts, Tt, Ti], err)
...
def sched(pointers, in_degrees, input_queue, arrival_order, arrival_times_actual, opt, sort, t_promotion = 1000_000000):
    adapt = True
    alpha = 0.00000001
    max_window = 128
    max_bs = 32
...
class Sampler():
    ...
    def sample_block(self, seeds):
        num_chunks = min(max(1, int((seeds.shape[0]/self.chunk_size)**2)), 50)
...
def main(number_of_requests, arrival_model, arrival_rate, num_layers, opt, sort, chunking, cache_size, GNN, pipeline, verbose, graph, feats, nsz, chunk_size):
    use_mps = pipeline
    ...
    if pipeline:
        ...
        inference_process = mp.Process(target = inference, args = (file_name, input_queue, graph, pointers, infer_en, arrival_times_actual, num_requests, barrier, arrival_order, res, in_degrees, feats, num_layers, opt, sort, chunking, cache_size, GNN, verbose, nsz, chunk_size))       
...
def find_percentiles(graph, feats, opts, num_layers, n_l1):
    device = 'cuda:0'
    chunk_size = 99999999999
    num_nodes = graph.number_of_nodes()
    if not(feats.is_pinned()):
        cudart = torch.cuda.cudart()
        rt = cudart.cudaHostRegister(feats.data_ptr(), feats.numel() * feats.element_size(), 0)
    ...
    if not(U.is_pinned()):
        dgl.utils.pin_memory_inplace(U)
    sampler = Sampler(num_layers - 1, graph, 'cuda:0', chunk_size = chunk_size)
    in_degs = graph.in_degrees()
    in_degs_gpu = in_degs.to(device = device)
    nids = np.random.choice(num_nodes, size = 1000000000, replace = True)
    for k in trange(len(opts)):
        p = 0
        opt = opts[k]
        n_lk = torch.zeros(n_l1.shape[0], dtype = torch.long)
        for i in range(n_l1.shape[0]):
            max_bs = 128
            bs = 0
            deg_count = 0
            for j in range(max_bs):
                deg_count += in_degs[nids[p+j]]
                if deg_count > opt:
                    break
                bs += 1
            bs = max(bs, 1)
            seeds = nids[p:p+bs]
            seeds = torch.from_numpy(seeds).to(device = device)
            p += bs  
            minibatch = sampler.sample(seeds)
            ins = minibatch[0]
            deg_sum = torch.sum(in_degs_gpu[ins])
            n_lk[i] = deg_sum
        n_l1[:, k] = torch.sort(n_lk)[0].detach().cpu()
...
if __name__ == '__main__':
    ...
    cache_size = 25000000
    opts = [100, 200, 400, 800, 1600]
    n_l1 = torch.zeros((10000, len(opts)), dtype = torch.long)
    n_l1.share_memory_()
    p = mp.Process(target=find_percentiles, args=(graph, feats, opts, num_layers, n_l1))
    p.start()
    p.join()
    step = n_l1.shape[0] // 20
    for i in range(20):
        print(i*step, n_l1[i*step,:])
    percentiles = {}
    for i in range(len(opts)):
        percentiles[opts[i]] = n_l1[:, i]
    with open ('pcts_PA_P.pkl', 'wb') as f:
