...
def inference(feats_file_name, input_queue, graph, pointers, infer_en, arrival_times_actual, num_requests, barrier, arrival_order, res, in_degrees, feats, num_layers, opt, sort, chunk_size, cache_size, GNN, verbose, nsz, lat):
    ...
    n_h = 0
    n_t = 0
    input_features = torch.empty((max(int(chunk_size/4) + 7500000, 5000000), feats.shape[1]), dtype = feats.dtype, device = 'cuda:0')
    torch.cuda.synchronize()
    ...
    if not(failure):
        ...
        if (e < l/1000):
            ...
            print('failure too many dropped requests. ', l, [e, e0, e1], n_h/n_t) 
...
if __name__ == '__main__':
    ...
    for i in range(len(arrival_rates)):
        ...
        res[i, 1:] = temp[argmin,:]
        print(res[i])
    file_name = "res_PA_M_ab_SAGE.pkl"
    with open (file_name, 'wb') as f:
        pickle.dump(res, f) 
