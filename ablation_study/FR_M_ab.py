...
def inference(feats_file_name, input_queue, graph, pointers, infer_en, arrival_times_actual, num_requests, barrier, arrival_order, res, in_degrees, feats, num_layers, opt, sort, chunk_size, cache_size, GNN, verbose, nsz, lat):
    ...
    n2 = 0
    er_c = 0
    input_features = torch.empty((max(int(chunk_size/9.5)+7500000, 5000000), feats.shape[1]), dtype = feats.dtype, device = 'cuda:0')
    torch.cuda.synchronize()
    ...
    if not(failure):
        ...
        if (e < l/1000):
            ...
            lat[7] = Ti
            lat[8] = l/n2
            print('Server: average latency = ', lat[0].item(), '(ms) failures =', [e, e0, e1], [Ts, Tt, Ti], n1, n2, sampler.t_c, mem_m/1024/1024, n_h/n_t)
        else:
...
if __name__ == '__main__':
    ...
    #arrival_rates = [100, 300, 340, 380, 390]
    arrival_rates = [390]
    ...
    for i in range(len(arrival_rates)):
        ...
        for j in range(cnds.shape[1]):
            ...
            opt = cnd[0].item()
            cache_size = 0.98*(torch.cuda.get_device_properties(0).total_memory) - cnd[2].item()
            cache_size = int(cache_size/(feats.shape[1] * feats.element_size()))
            number_of_requests = time * arrival_rate
            if (cache_size > 0) and (j > 0) and (j < cnds.shape[1]-2):
        ...
        res[i, 1:] = temp[argmin,:]
        print(res[i])
    file_name = "res_FR_M_ab_SAGE.pkl"
    with open (file_name, 'wb') as f:
        pickle.dump(res, f) 
