...
def inference(feats_file_name, input_queue, graph, pointers, infer_en, arrival_times_actual, num_requests, barrier, arrival_order, res, in_degrees, feats, num_layers, opt, sort, chunk_size, cache_size, GNN, verbose, nsz, lat):
    ...
    n_h = 0
    n_t = 0
    input_features = torch.empty((max(int(chunk_size/4.5) + 5500000, 5000000), feats.shape[1]), dtype = feats.dtype, device = 'cuda:0')
    torch.cuda.synchronize()
    ...
    if not(failure):
        ...
        al = ((torch.mean(((response_times - arrival_times_actual)/1000_000).detach()))).to(dtype = torch.float)
        if (e < l/1000):
            ...
            print('Server: average latency = ', al, '(ms) failures =', [e, e0, e1], [Ts, Tt, Ti], n1, n2, sampler.t_c, mem_m/1024/1024)
...
if __name__ == '__main__':
    ...
    arrival_rates = [241]
    arrival_rates = [100, 150, 175, 200, 220, 235, 241]
    arrival_rates = [100, 150, 200, 210, 219, 230, 235, 240, 250]
    ...
    arrival_rates = []
    #arrival_rates = [37]
    #arrival_rates = [10, 20, 25, 30, 35, 36, 37]
    #arrival_rates = [10, 20, 30, 34, 36, 37, 38]
    #res = torch.zeros((len(arrival_rates), 10), dtype = torch.float)
    #temp = torch.zeros(cnds.shape[1], 9)
    for i in range(len(arrival_rates)):
        print('\n')
        ...
        for j in range(cnds.shape[1]):
            ...
            chunk_size = int(cnd[1].item())
            opt = cnd[0].item()
            cache_size = 0.92*(torch.cuda.get_device_properties(0).total_memory) - cnd[2].item()
            cache_size = int(cache_size/(feats.shape[1] * feats.element_size()))
            number_of_requests = time * arrival_rate
            if (cache_size > 0) and (j > -1) and (j < cnds.shape[1]):
                print(arrival_rate, 'sort, ', opt, chunk_size, cache_size, GNN)
