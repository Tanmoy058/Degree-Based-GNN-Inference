...
def inference(feats_file_name, input_queue, graph, pointers, infer_en, arrival_times_actual, num_requests, barrier, arrival_order, res, in_degrees, feats, num_layers, opt, sort, chunk_size, cache_size, GNN, verbose, nsz, lat):
    ...
    n_h = 0
    n_t = 0
    input_features = torch.empty((max(int(chunk_size/4.5)+ 4000000, 5000000), feats.shape[1]), dtype = feats.dtype, device = 'cuda:0')
    torch.cuda.synchronize()
...
if __name__ == '__main__':
    ...
    arrival_rates = [240]
    arrival_rates = [100, 150, 175, 200, 220, 230, 236, 240]
    arrival_rates = [100, 150, 200, 210, 219, 230]
    arrival_rates = [230]
    ...
    GNN = 'GAT'
    arrival_rates = [39]
    arrival_rates = [10, 20, 25, 30, 34, 36, 39, 40]
