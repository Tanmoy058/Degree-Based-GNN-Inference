...
def inference(feats_file_name, input_queue, graph, pointers, infer_en, arrival_times_actual, num_requests, barrier, arrival_order, res, in_degrees, feats, num_layers, opt, sort, chunk_size, cache_size, GNN, verbose, nsz, lat):
    ...
    torch.cuda.synchronize()
    cache_meta[cached_feats] = torch.from_numpy(np.arange(cache_size, dtype = np.int64)).to(device = 'cuda:0')%2000000
    torch.cuda.synchronize()
    ...
    n_h = 0
    if GNN == 'SAGE':
        input_features = torch.empty((max(int(chunk_size/3.0)+ 3700000, 5500000), feats.shape[1]), dtype = feats.dtype, device = 'cuda:0')
    elif GNN == 'GAT':
        input_features = torch.empty((max(int(chunk_size/4.5)+ 4000000, 5500000), feats.shape[1]), dtype = feats.dtype, device = 'cuda:0')        
    torch.cuda.synchronize()
            ...
            while infer_en[0]:
                while infer_en[0]:
                    if pointers[0]>pointers[1]:
                        ...
                        sched(pointers, in_degrees, input_queue, arrival_order, arrival_times_actual, opt, sort, GNN, load)
...
def sched(pointers, in_degrees, input_queue, arrival_order, arrival_times_actual, opt, sort, GNN, load):
    adapt = True
    alpha = 0.00000001
    max_window = 100000
    max_bs = 64
...
if __name__ == '__main__':
    ...
    wr = []
    arrival_rates = []
    #arrival_rates = [100, 200, 219, 240, 250]
    #arrival_rates = [250, 255]
    arrival_rates = [100, 150, 200, 210, 219, 230, 235, 240, 250, 255]
    #arrival_rates = [255]
    ...
    for i in range(len(arrival_rates)):
        ...
        res[i, 1:] = temp[argmin,:]
        #print(res[i])
    file_name = "r_PA_P_SAGE_overhead.pkl"
    with open (file_name, 'wb') as f:
        pickle.dump(wr, f) 
    ...
    with open(file_name, 'rb') as f:
        cnds = pickle.load(f)
    GNN = 'GAT'
    arrival_rates = [10, 30, 34, 36, 39, 40]
    arrival_rates = [10, 20, 30, 34, 36, 37, 38, 39, 40]
    #arrival_rates = [40]
    res = torch.zeros((len(arrival_rates), 10), dtype = torch.float)
    temp = torch.zeros(cnds.shape[1], 9)
    for i in range(len(arrival_rates)):
        ...
        for j in range(cnds.shape[1]):
            ...
            cache_size = int(cache_size/(feats.shape[1] * feats.element_size()))
            number_of_requests = 100 * arrival_rate
            if (cache_size > 0) and (j > 0) and (j < cnds.shape[1]):
                print(arrival_rate, 'sort, ', opt, chunk_size, cache_size, GNN)
                p = mp.Process(target = main, args = (number_of_requests, arrival_model, arrival_rate, num_layers, opt, True, chunk_size, cache_size, GNN, pipeline, verbose, graph, feats, 10000000, lat))                
                p.start()
                p.join()
        ...
        res[i, 1:] = temp[argmin,:]
        print(res[i])
    file_name = "r_PA_P_GAT_overhead.pkl"
    with open (file_name, 'wb') as f:
        pickle.dump(wr, f) 
