...
def inference(feats_file_name, input_queue, graph, pointers, infer_en, arrival_times_actual, num_requests, barrier, arrival_order, res, in_degrees, feats, num_layers, opt, sort, chunk_size, cache_size, GNN, verbose, nsz, lat):
    ...
    n_t = 0
    n_h = 0
    input_features = torch.empty((max((int(chunk_size/6.5 + 5500000)), 5000000), feats.shape[1]), dtype = feats.dtype, device = 'cuda:0')
    torch.cuda.synchronize()
...
if __name__ == '__main__':
    ...
    #arrival_rates = [100, 200, 300, 330, 340, 344, 500, 530, 535, 540]
    arrival_rates = [100, 360, 364, 550, 580, 590]
    arrival_rates = [100, 200, 300, 350, 364, 450, 480, 490, 540, 550, 561, 580, 592]
    #arrival_rates = [592]
    ...
    for i in range(len(arrival_rates)):
        ...
        res[i, 1:] = temp[argmin,:]
        print(res[i])
    file_name = "r_FR_P_SAGE_overhead.pkl"
    with open (file_name, 'wb') as f:
        pickle.dump(wr, f) 
    ...
    with open(file_name, 'rb') as f:
        cnds = pickle.load(f)
    GNN = 'GAT'
    arrival_rates = [20, 40, 60, 68, 71 , 80, 86, 89, 95]
    #arrival_rates = [100]
    ...
    for i in range(len(arrival_rates)):
        ...
        res[i, 1:] = temp[argmin,:]
        print(res[i])
    file_name = "r_FR_P_GAT_overhead.pkl"
    with open (file_name, 'wb') as f:
        pickle.dump(wr, f)
