            ...
            while infer_en[0]:
                ...
                try:
                    ...
                    for l in range(len(minibatch[2]) - num_layers + 1):
                        torch.cuda.synchronize()
                        t1 = time.perf_counter()
                        if torch.cuda.memory_allocated() > 1024*1024*1024*36:
                            gc.collect()
                            torch.cuda.empty_cache()
                        n2 += 1
                        block = minibatch[2][l]
...
if __name__ == '__main__':
    ...
    opts = list(pcts.keys())
    cnds = torch.zeros((2,len(opts), 3), dtype = torch.long)
    max_mem = torch.zeros(1, dtype = torch.long)
    max_mem.share_memory_()
    base_config = [170, 200, 1000000, False, 5000000, 10500000]
    for i in range(len(opts)):
        max_mem[0] = 999999999999
        if i>0 and cnds[0, i-1, 2] > 199999999999:
            cnds[0,i,0] = opt
            cnds[0,i,1] = chunk_size
            cnds[0,i,2] = max_mem[0]
            continue
        config = [base_config[i] for i in range(len(base_config))]
        opt = opts[i]
        pcts_i = pcts[opt]
        chunk_size = pcts_i[int(0.998*pcts_i.shape[0])]
        config[1] = opt
        config[4] = chunk_size
        print(config)
        try:
        ...
        cnds[0,i,1] = chunk_size
        cnds[0,i,2] = max_mem[0]
    with open ('lin_PA_B.pkl', 'wb') as f:
        pickle.dump(cnds, f)         
        
    GNN = 'GAT' 
    base_config = [50, 200, 1000000, False, 5000000, 10500000]   
    for i in range(len(opts)):
        max_mem[0] = 999999999999
        if i>0 and cnds[1, i-1, 2] > 199999999999:
            cnds[1,i,0] = opt
            cnds[1,i,1] = chunk_size
            cnds[1,i,2] = max_mem[0]
            continue
        config = [base_config[i] for i in range(len(base_config))]
        opt = opts[i]
        pcts_i = pcts[opt]
