...
def find_percentiles(graph, opts, num_layers, n_l1):
    device = 'cuda:0'
...
if __name__ == '__main__':
    ...
    n_l1 = torch.zeros((5000, len(opts)), dtype = torch.long)
    n_l1.share_memory_()
    p = mp.Process(target=find_percentiles, args=(graph, opts, num_layers, n_l1))
    p.start()
    p.join()
    ...
    with open ('pcts_FR_P.pkl', 'wb') as f:
