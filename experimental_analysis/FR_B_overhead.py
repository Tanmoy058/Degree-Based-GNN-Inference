...
def inference(feats_file_name, input_queue, graph, pointers, infer_en, arrival_times_actual, num_requests, barrier, arrival_order, res, in_degrees, feats, num_layers, opt, sort, chunk_size, cache_size, GNN, verbose, nsz, lat):
        ...
        with torch.no_grad():
            while infer_en[0]:
                ...
                try:
                    ...
                    seeds = dim1(seeds.clone().to(device = 'cuda:0'))
                    for s_i in range(seeds.shape[0]):
                        s = seeds[s_i]
                ...
                saved_data[p_w, 2] = t3 - t2
                saved_data[p_w, 3] = sampler.t_c
                saved_data[p_w, 4] = t4 - t3
                saved_data[p_w, 5] = t5 - t4
                saved_data[p_w, 6] = t5 - t0
                saved_data[p_w, 7] = ins.shape[0]
                saved_data[p_w, 8] = torch.cuda.max_memory_allocated()
                sampler.t_c = 0
                torch.cuda.reset_peak_memory_stats()
    ...
    file_name = 'FR_'+ str(GNN) + '_' + str(opt) + 'o_.pkl'
