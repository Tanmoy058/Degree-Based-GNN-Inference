...
def inference(feats_file_name, input_queue, graph, pointers, infer_en, arrival_times_actual, num_requests, barrier, arrival_order, res, in_degrees, feats, num_layers, opt, sort, chunk_size, cache_size, GNN, verbose, nsz, lat):
    ...
    mem_usage = []
    Time = []
    mem_0 = torch.cuda.max_memory_allocated()
    torch.cuda.reset_peak_memory_stats()
    if GNN == 'SAGE':
        model = SAGE2(feats.shape[1],256, 100, num_layers).to(device = 'cuda:0')
        sampler = MySampler(num_layers, graph, chunk_size = chunk_size, compact_threshold = 3000000)
    elif GNN == 'GAT':
    ...
    n2 = 0
    er_c = 0
    input_features = torch.empty((max(int(chunk_size/10)+6500000, 5000000), feats.shape[1]), dtype = feats.dtype, device = 'cuda:0')
    torch.cuda.synchronize()
    torch.cuda.reset_peak_memory_stats()
    barrier.wait()
    T_st = time.time()
    saved_data = torch.zeros((20000, 9), dtype=torch.float64)
    p_w = 0 
    try:
        with torch.no_grad():
            while infer_en[0]:
                ...
                try:
                    ...
                    if cnt:
                        ...
                        outs = model(minibatch[2], input_features[0:ins.shape[0]], op = 0)
                        er_c = 4
                        torch.cuda.synchronize()
                ...
                pointers[2] = p + bs
                saved_data[p_w, 0] = t1 - t0
                saved_data[p_w, 1] = t2 - t1
                saved_data[p_w, 2] = sampler.t_c
                saved_data[p_w, 3] = t3 - t2
                saved_data[p_w, 4] = t4 - t3
                saved_data[p_w, 5] = t4 - t0
                saved_data[p_w, 6] = ins.shape[0]
                saved_data[p_w, 7] = torch.cuda.max_memory_allocated()
                saved_data[p_w, 8] = minibatch[2][-1].number_of_edges()
                sampler.t_c = 0
                torch.cuda.reset_peak_memory_stats()
        ...
        failure = True
    #print(Ts, Tt, Ti)
    saved_data = saved_data[0:p_w-1]
    data = [[e, e0, e1, l], opt, GNN, p_w, saved_data]
    file_name = 'PA_'+ str(GNN) + '_' + str(opt) + '_p.pkl'
    with open (file_name, 'wb') as f:
        pickle.dump(data, f)
    print(torch.mean(saved_data, dim=0), p_w)
    if not(failure):
        l = num_requests[0]
        l -= 1
...
def sched(pointers, in_degrees, input_queue, arrival_order, arrival_times_actual, opt, sort, GNN):
    ...
    if False:
        ...
        p = max(min(degs_q.shape[0], opt), 1)
...
if __name__ == '__main__':
    ...
    for i in range(len(arrival_rates)):
        ...
        for j in range(cnds.shape[1]):
            ...
            opt = cnd[0].item()
            cache_size = 0.95*(torch.cuda.get_device_properties(0).total_memory) - cnd[2].item()
            print(cache_size/1024/1024/1024, torch.cuda.get_device_properties(0).total_memory/1024/1024/1024)
            cache_size = int(cache_size/(feats.shape[1] * feats.element_size()))
            number_of_requests = 8000
    ...
    for i in range(len(arrival_rates)):
        ...
        for j in range(cnds.shape[1]):
            ...
            opt = cnd[0].item()
            cache_size = 0.95*(torch.cuda.get_device_properties(0).total_memory) - cnd[2].item()
            cache_size = max(int(cache_size/(feats.shape[1] * feats.element_size())), 100000)
            number_of_requests = 8000
            if (cache_size > 0) and (j > -1) and (j < cnds.shape[1]-1):
                print(arrival_rate, 'sort, ', opt, chunk_size, cache_size, GNN)
                p = mp.Process(target = main, args = (number_of_requests, arrival_model, arrival_rate, num_layers, opt, True, chunk_size, cache_size, GNN, pipeline, verbose, graph, feats, 10000000, lat))                
