...
def inference(feats_file_name, input_queue, graph, pointers, infer_en, arrival_times_actual, num_requests, barrier, arrival_order, res, in_degrees, feats, num_layers, opt, sort, chunking, cache_size, GNN, verbose, nsz, max_mem):
    ...
    if chunk_size < 1000000000:
        input_features = torch.empty((max(int(chunk_size/3)+ 4500000, 5000000), feats.shape[1]), dtype = feats.dtype, device = 'cuda:0')
    else:
...
if __name__ == '__main__':
    ...
    opts = list(pcts.keys())
    cnds = torch.zeros((2,5,3), dtype = torch.long)
    max_mem = torch.zeros(1, dtype = torch.long)
    max_mem.share_memory_()
    base_config = [200, 200, 1000000, False, 5000000, 10500000]
    for k in range(5):
        i = 3
        ...
        opt = opts[i]
        pcts_i = pcts[opt]
        chunk_size = int(pcts_i[int(0.80*pcts_i.shape[0])] * (0.50+k*0.25))
        config[1] = opt
        config[4] = chunk_size
        print(config)
        try:
        ...
        cnds[0,k,0] = opt
        cnds[0,k,1] = chunk_size
        cnds[0,k,2] = max_mem[0]
    with open ('sens_PA_P.pkl', 'wb') as f:
        pickle.dump(cnds, f)         
        
    GNN = 'GAT'
    base_config = [100, 200, 1000000, False, 5000000, 10500000]
    for k in range(5):
        i = 0
        max_mem[0] = 999999999999
        config = [base_config[i] for i in range(len(base_config))]
        opt = opts[i]
        pcts_i = pcts[opt]
        chunk_size = int(pcts_i[int(0.97*pcts_i.shape[0])] * (0.50+k*0.25))
        config[1] = opt
        config[4] = chunk_size
        print(config)
        try:
        ...
        print('max memory usage = ', max_mem[0])
        cnds[1,k,0] = opt
        cnds[1,k,1] = chunk_size
        cnds[1,k,2] = max_mem[0]
