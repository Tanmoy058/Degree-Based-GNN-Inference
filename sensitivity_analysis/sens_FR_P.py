...
def inference(feats_file_name, input_queue, graph, pointers, infer_en, arrival_times_actual, num_requests, barrier, arrival_order, res, in_degrees, feats, num_layers, opt, sort, chunking, cache_size, GNN, verbose, nsz, max_mem):
    ...
    if chunk_size < 1000000000:
        input_features = torch.empty((max(int(chunk_size/4.5)+ 3000000, 5000000), feats.shape[1]), dtype = feats.dtype, device = 'cuda:0')
    else:
...
if __name__ == '__main__':
    ...
    for k in range(5):
        ...
        opt = opts[i]
        pcts_i = pcts[opt]
        chunk_size = int(pcts_i[int(0.80*int(pcts_i.shape[0]))] * (0.50+k*0.25))
        config[1] = opt
        config[4] = chunk_size
        print(config)
        try:
    ...
    for k in range(5):
        ...
        opt = opts[i]
        pcts_i = pcts[opt]
        chunk_size = int(pcts_i[int(0.97*int(pcts_i.shape[0]))] * (0.50+k*0.25))
        config[1] = opt
        config[4] = chunk_size
        print(config)
        try:
